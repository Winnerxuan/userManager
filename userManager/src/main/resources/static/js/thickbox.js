		...
		if(urlType == '.jpg' || urlType == '.jpeg' || urlType == '.png' || urlType == '.gif' || urlType == '.bmp'){//code to show images
			...
			if(url.indexOf('TB_iframe') != -1){				
					...
					if($("#TB_window").css("display") != "block"){
						...
						$("#TB_window").append("<div id='TB_title'><div id='TB_ajaxWindowTitle'>"+caption+"</div><div id='TB_closeAjaxWindow'><a href='#' id='TB_closeWindowButton' title='Close'>[关闭]</a></div></div><div id='TB_ajaxContent' style='width:"+ajaxContentW+"px;height:"+ajaxContentH+"px'></div>");
						}else{
						$("#TB_overlay").unbind();
...
function tb_remove() {
	...
	$("#TB_window,#TB_overlay,#TB_HideSelect").hide().remove();
	//$("#TB_window").fadeOut("fast",function(){$('#TB_overlay,#TB_HideSelect').remove();$("#TB_window").remove();});
	$("#TB_load").remove();
	if (typeof document.body.style.maxHeight == "undefined") {//if IE 6
