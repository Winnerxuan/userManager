...
(function(document, window) {
	...
	window.inlineAttach = function(options, instance) {
		...
		var settings = merge(options, inlineAttach.defaults), editor = instance, filenameTag = '{filename}', lastValue, me = this;
		...
		this.uploadFile = function(file) {
			...
			if (settings.addFileBeforeExtraParameters) {
				formData.append(settings.uploadFieldName, file, "image-"
						+ Date.now() + "." + extension);
			}
		...
		this.parseResponse = function(xhr) {
			return settings.customResponseParser.call(this, xhr)
		...
		this.onUploadedFile = function(data) {
			var result = settings.onUploadedFile.call(this, data), filename;
		 ...
		 */
		this.onPaste = function(e, input) {

			var result = false, clipboardData = e.clipboardData,
		...
		this.waitforpastedata = function(savedcontent) {
	...
	window.inlineAttach.attachToInput = function(input, options) {
		...
		var inlineattach = new inlineAttach(options, editor);
		try {
			if (input.addEventListener) // W3C DOM
			{
				input.addEventListener('paste', function(e) {
					inlineattach.onPaste(e, input);
				}, false);
			...
			} else if (input.attachEvent) { // IE DOM
